...
async function translateCues(cues, config, progress) {
    ...
    async function flushBatch() {
        ...
        try {
            ...
            // source text). Keep going so we make as much progress as possible this run, but
            // remember the first cause so the caller can explain it instead of guessing.
            failure = failure || { reason: err.reason || "error", message: err.message };
    ...
    return { translations: translated, complete: !failure, failure };
}
