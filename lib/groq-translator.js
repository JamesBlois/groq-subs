...
async function translateGroqBatch(texts, config) {
    ...
    const fallbackModels = [primary, ...GROQ_MODELS.filter((m) => m !== primary)];
    ...
    for (let i = 0; i < chunks.length; i += 1) {
        ...
        const result = await translateChunkWithFallback(chunks[i], config, apiKey, fallbackModels);
...
async function translateChunkWithFallback(chunk, config, apiKey, fallbackModels) {
    ...
    logger.error("All Groq models failed for chunk, returning source text", { chunkSize: chunk.length });
    return chunk;
}
