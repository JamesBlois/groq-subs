...
// Carries why translation failed (and what every model answered) so callers can tell a bad API
// key apart from an exhausted quota instead of reporting every failure as "rate-limited".
class TranslationError extends Error {
    constructor(message, { reason, failures = [] } = {}) {
        super(message);
...
// Worst-first: an invalid key or a blocked account needs the user to act, whereas a rate limit
// resolves itself, and a mismatch means the models answered but broke cue alignment.
function classifyFailures(failures) {
    const reasons = new Set(failures.map((failure) => failure.reason));
    for (const reason of ["invalid_key", "blocked", "rate_limited", "network", "http_error", "mismatch"]) {
...
function summarizeFailures(failures) {
    return failures.map((failure) => `${failure.model}: ${failure.reason}${statusSuffix(failure)}`).join("; ");
}

function statusSuffix(failure) {
    const details = [failure.status ? `status ${failure.status}` : "", failure.message || ""].filter(Boolean);
...
async function testGroqApiKey(config) {
    ...
    try {
        ...
        const { response, data } = await streamChatCompletion({
            ...
            options: { temperature: 0, maxTokens: 5 },
        });
        ...
        // Without this the only trace of a failed key test is the HTTP response to the browser.
        logger.warn("Groq API key test failed", { error, model });
...
async function translateGroqBatch(texts, config) {
    ...
    if (pool.length === 0) {
        ...
        throw new TranslationError("Missing API key for translation", { reason: "missing_key" });
    }
...
async function translateChunkWithFallback(batch, config, fallbackModels, previousTranslations) {
    ...
    async function tryModel(entry) {
        ...
        if (breaker.isOpen(modelKey)) {
            ...
            return { ok: false, reason: "rate_limited", message: "in rate-limit cooldown" };
        }
        ...
        for (const sub of subChunks) {
            ...
            if (translatedLines.length !== sub.length) {
                ...
                return {
                    ok: false,
                    reason: "mismatch",
                    message: `expected ${sub.length} lines, received ${translatedLines.length}`,
                };
            }
    ...
    for (let attempt = 0; attempt < fallbackModels.length; attempt += 1) {
        ...
        const model = `${entry.providerId}:${entry.model}`;
        try {
            ...
            failures.push({ model, reason: result.reason, status: result.status, message: result.message });
        } catch (err) {
            logger.warn("fetch error, trying next model", {
                error: err,
                model,
            });
            failures.push({ model, reason: "network", message: err.message });
        }
    }

    const reason = classifyFailures(failures);
    logger.error("All models failed for batch", {
        batchSize: batch.length,
        failures,
        reason,
    });
    throw new TranslationError(`All models failed for chunk (${reason}): ${summarizeFailures(failures)}`, {
        failures,
        reason,
    });
}
