const { envList, envPositiveNumber, envString } = require("./env");
const logger = require("./logger");
const { LANGUAGES } = require("./languages");
const { isAbortError, streamChatCompletion } = require("./llm-stream");
...
// Minimal request used to check that a key/model pair answers at all.
const PING_MESSAGES = [
    { role: "system", content: "You are a translation assistant. Reply with the single word: ok" },
    { role: "user", content: "ping" },
];
const PING_OPTIONS = { temperature: 0, maxTokens: 5 };
// Per-probe ceiling so one dead model cannot block the whole model-status dashboard.
...
function groqApiKey(config) {
    return (config?.groqApiKey || envString("GROQ_API_KEY")).trim();
}

function groqModel(config) {
    const model = (config?.groqModel || envString("GROQ_MODEL") || DEFAULT_GROQ_MODEL).trim();
    return model || DEFAULT_GROQ_MODEL;
}

// Each provider exposes an OpenAI-compatible chat completions endpoint + a model list. The
// generic provider (e.g. OpenRouter, NVIDIA NIM, Cerebras, DeepSeek) is configured via env so
// the user can add free model sources to the rotation pool.
function getProviders(config) {
    ...
    const models = envList("LLM_MODELS");
...
async function testGroqApiKey(config) {
    ...
    try {
        // Stream the probe so a slow-to-first-token model (NVIDIA NIM, large Groq models) does
        // not hang or time out the key test. A 429 here means the KEY is valid but the chosen
        // model is momentarily rate-limited — not a bad key. We surface that distinctly so the
        // UI can say "key ok, model rate-limited" instead of "API key không hợp lệ".
        ...
        if (response.status === 429) {
            // Key accepted by the API; only this model is rate-limited right now. The addon
            // still works (it falls back across all models), so treat the key as valid.
            return {
                ok: true,
                status: 429,
                rateLimited: true,
                model,
                message: `Key hợp lệ, nhưng model "${model}" đang giới hạn tốc độ. Thử model khác — addon vẫn fallback tự động.`,
            };
        }
...
// Map an HTTP failure status onto the model state reported by /models-status.
function modelStateForStatus(status) {
...
// Probe one (provider, model) with a tiny STREAMING request, resolving on the first streamed
// bytes. Waiting for a full completion would report live-but-slow models (large NVIDIA models)
// as timed out, because their time-to-first-token exceeds a non-streaming wait.
async function probeModel(provider, model) {
    const modelKey = `${provider.id}:${model}`;
    // Skip the live probe for models already known to be in rate-limit cooldown: their state is
    // already known and probing them just wastes a slow request.
    ...
    const timeoutMs = envPositiveNumber("PROBE_TIMEOUT_MS", DEFAULT_PROBE_TIMEOUT_MS);
    try {
        const { response, data } = await streamChatCompletion({
        ...
        return {
            provider: provider.id,
            model,
            state: response.ok ? "available" : modelStateForStatus(response.status),
            circuitOpen: breaker.isOpen(modelKey),
            message: response.ok ? undefined : data.error?.message,
            status: response.status,
        };
    } catch (error) {
        ...
        return {
            ...
            circuitOpen: breaker.isOpen(modelKey),
            message: timedOut ? `no response within ${timeoutMs / 1000}s` : error.message,
            status: 0,
        };
    }
}
    ...
    async function tryModel(entry) {
        ...
        for (const sub of subChunks) {
            ...
            const requestTranslation = () =>
                chatCompletion(entry.baseUrl, entry.apiKey, entry.model, systemInstruction, userPrompt);

            let { response, data } = await requestTranslation();
            ...
            if (response.status === 429) {
                ...
                if (retryAfterMs !== null && retryAfterMs <= RETRY_BACKOFF_MAX_MS) {
                    ...
                    ({ response, data } = await requestTranslation());
                }
            }
